...
if ( in_array( 'rvm-custom-marker-icon/rvm-custom-marker-icon.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
    ...
    if ( !empty( $rvm_options[ 'rvm_custom_icon_marker_module_path_verified' ] ) ) {
        if ( rvm_is_marker_module_in_download_dir_yet( $rvm_options[ 'rvm_custom_icon_marker_module_path_verified' ] ) ) {
            ...
            $output .= '<div class="rvm_messages rvm_error_messages">' . __( 'It seems there is no marker module installed or maybe was deleted. Please reinstall it using RVM global settings in Settings menu', 'responsive-vector-maps' ) . '</div>';
        }
        ...
        $output .= '<div class="rvm_messages rvm_notice_messages rvm_marker_messages"><img src="' . RVM_IMG_PLUGIN_DIR . '/map-icon-16x16.png' . '" alt="Map pintpoint icon"/>' . __( 'Tired to use the default circle icon for Markers\' pinpoints? This is the right time to <a href="https://www.responsivemapsplugin.com/redirect-to-marker-icon-module-from-plugin-dashboard/" target="_blank">download the Custom Icon Marker Module!</a>', 'responsive-vector-maps' ) . '</div>';
    }
} //if( in_array( 'rvm-custom-marker-icon/rvm-custom-marker-icon.php', apply_filters( 'active_plugins', get_option('active_plugins') ) ) )
//End : marker uploader  
$output .= '<div id="rvm_mbe_fields_wrap">';
$output .= '<h2 class="rvm_h2_title">' . __( 'Create New Markers', 'responsive-vector-maps' ) . '</h2>';
$output .= '<input type="button" id="rvm_mbe_add_field_button" class="button-primary" value="' . __( 'Add Markers', 'responsive-vector-maps' ) . '" >';
...
$rvm_marker_array_count = ( is_array( $marker_array_unserialized[ 'rvm_marker_name_array' ] ) && $marker_array_unserialized[ 'rvm_marker_name_array' ] ) ? count( $marker_array_unserialized[ 'rvm_marker_name_array' ] ) : 0; // count element of the array starting from 1
if ( is_array( $marker_array_unserialized[ 'rvm_marker_name_array' ] ) && $rvm_marker_array_count > 0 ) {
    $rvm_is_marker = 1;
    $output .= '<h4 class="rvm_h4_title rvm_added_markers_title">' . __( 'Added Markers', 'responsive-vector-maps' ) . '<span>' . __( ' ( Not visible in map preview )', 'responsive-vector-maps' ) . '</span></h4>';
    for ( $i = 0; $i < $rvm_marker_array_count; $i++ ) {
        $output .= '<div class="rvm_markers">';
        $output .= '<p><label for="marker_name" class="rvm_label rvm_label_markers">' . __( 'Name', 'responsive-vector-maps' ) . '*</label><input type="text" name="rvm_marker_name[]" value="' . wp_unslash( $marker_array_unserialized[ 'rvm_marker_name_array' ][ $i ] ) . '" /></p>';
        $output .= '<p><label for="marker_lat" class="rvm_label rvm_label_markers">' . __( 'Latitude', 'responsive-vector-maps' ) . '*</label><input type="text" name="rvm_marker_lat[]" value="' . $marker_array_unserialized[ 'rvm_marker_lat_array' ][ $i ] . '" placeholder="e.g. 48.921537" /></p>';
        $output .= '<p><label for="marker_long" class="rvm_label rvm_label_markers">' . __( 'Longitude', 'responsive-vector-maps' ) . '*</label><input type="text" name="rvm_marker_long[]" value="' . $marker_array_unserialized[ 'rvm_marker_long_array' ][ $i ] . '" placeholder="e.g. -66.829834" /></p>';
        $output .= '<p><label for="marker_link" class="rvm_label rvm_label_markers">' . __( 'Link', 'responsive-vector-maps' ) . '</label><input type="text" name="rvm_marker_link[]" value="' . esc_url( $marker_array_unserialized[ 'rvm_marker_link_array' ][ $i ] ) . '" /></p>';
        $output .= '<p><label for="marker_dim" class="rvm_label rvm_label_markers">' . __( 'Dimension', 'responsive-vector-maps' ) . '<br><span class="rvm_small_text">' . __( 'Use only integer or decimal', 'responsive-vector-maps' ) . '</span></label><input type="text" name="rvm_marker_dim[]" value="' . $marker_array_unserialized[ 'rvm_marker_dim_array' ][ $i ] . '" placeholder="' . __( 'e.g. 591.20', 'responsive-vector-maps' ) . '" /></p>';
        $output .= '<p><label for="marker_popup" class="rvm_label rvm_label_markers" style="vertical-align:top;">' . __( 'Html popup label', 'responsive-vector-maps' ) . '</label><textarea name="rvm_marker_popup[]" placeholder="' . __( 'e.g. Rome precipitation (mm) long term averages', 'responsive-vector-maps' ) . '" >' . wp_unslash( $marker_array_unserialized[ 'rvm_marker_popup_array' ][ $i ] ) . '</textarea></p>';
...
if ( is_array( $marker_array_unserialized[ 'rvm_marker_name_array' ] ) && ( $rvm_marker_array_count > 1 ) && ( $rvm_is_marker = 1 ) ) {
    ...
    $output .= '<input type="button" id="rvm_delete_all_markers_button" class="button-primary"  value=" ' . __( 'Delete all Markers', 'responsive-vector-maps' ) . '" />';
    $output .= '</div>';
}
...
if ( !rvm_check_custom_marker_icon_available( $rvm_custom_marker_icon_path ) || empty( $rvm_options[ 'rvm_custom_icon_marker_module_path_verified' ] ) ) {
    ...
    $output .= '<h2 class="rvm_h2_title">' . __( 'Markers Colours', 'responsive-vector-maps' ) . '</h2>';
...
$output .= '<h2 class="rvm_h2_title">' . __( 'Effects', 'responsive-vector-maps' ) . '</h2>';
...
if ( $rvm_is_marker && current_user_can( 'edit_others_posts' ) ) {
    $output .= '<h2 class="rvm_h2_title">' . __( 'Export Markers', 'responsive-vector-maps' ) . '</h2>';
    $output .= '<div class="rvm_export_markers_button_wrapper">';
    $output .= '<input type="button" id="rvm_export_markers_button" class="button-primary"  value=" ' . __( 'Export Markers', 'responsive-vector-maps' ) . '" />';
    //$output .= '<div id="rvm_export_markers_status" class="rvm_notice_messages"></div>' ;
    $output .= '</div>'; //.rvm_export_markers_button_wrapper
    $output .= '<div class="rvm_clear_left"></div>';
}
$output .= '<h2 class="rvm_h2_title">' . __( 'Import Markers  ( please use exported files from RVM only )', 'responsive-vector-maps' ) . '</h2>';
$output .= '<div class="rvm_import_markers_button_wrapper">';
$output .= '<input type="hidden" id="rvm_upload_markers_file_id" value=""/>';
$output .= '<input type="button" id="rvm_upload_markers_button" class="button-primary rvm_media_uploader"  value=" ' . __( 'Select Markers File', 'responsive-vector-maps' ) . '" />';
$output .= '<input type="button" id="rvm_import_markers_button" class="button-primary"  value=" ' . __( 'Import Markers', 'responsive-vector-maps' ) . '" />';
$output .= '<input type="button" id="rvm_import_reset_markers_button" class="button-secondary"  value=" ' . __( 'Restore old values', 'responsive-vector-maps' ) . '" onclick="window.location.reload();" />';
