...
{
    if ( substr( trim( $value ), 0, 4 ) === RVM_PREFIX) {
        $path = substr( trim( $value ), 4 );
    } //substr( trim( $value ), 0, 4 ) === RVM_PREFIX
    else {
        $path = $value;
    }
    return $path;
...
function rvm_retrieve_marker_icon_name( $file_uploaded )
{

    if ( realpath( get_attached_file( $file_uploaded ) ) ) {
        ...
        $marker_icon_name_array = explode( '/', trim( $file_uploaded ) );
        $marker_icon_name_array_count = count( $marker_icon_name_array );
        if ( $marker_icon_name_array_count > 2 ) {
            $marker_icon_name = $marker_icon_name_array[ $marker_icon_name_array_count - 3 ] . '/' . $marker_icon_name_array[ $marker_icon_name_array_count - 2 ] . '/' . $marker_icon_name_array[ $marker_icon_name_array_count - 1 ]; // i.e.: 2018/03/favicon.png
        } else {
            $marker_icon_name = get_attached_file( $file_uploaded );
        }
...
//Check validity of file uploaded
function rvm_is_valid_upload( $file_uploaded, $map_name, $section, $extension )
{
    $rvm_attached_file_path = get_attached_file( $file_uploaded );//get absolute file path
    $rvm_upload_file_ext = pathinfo( $rvm_attached_file_path, PATHINFO_EXTENSION );//i.e. csv
    $rvm_upload_file_name = pathinfo( $rvm_attached_file_path, PATHINFO_FILENAME );
    $rvm_is_valid_upload = ( realpath( $rvm_attached_file_path ) && $rvm_upload_file_ext == $extension && $rvm_upload_file_name == $map_name . '-' . $section) ? 1 : 0;
    return $rvm_is_valid_upload;
}
//Allowed tags for wp_kses() function
function rvm_allowed_tags( $context ) {
    if( $context === 'regions-import' || $context === 'markers-import' || $context === 'core' ||  $context === 'html-popup' ) {
        $allowedHtml = array(
            ...
            'img' => array( 'src' => array(), 'id' => array(), 'class' => array(), 'alt' => array(), 'width' => array(), 'height' => array() ),
            ...
            'strong' => array(),
            'table' => array( 'id' => array(), 'class' => array(), 'cellspacing' => array(), 'cellpadding' => array(), 'width' => array(), 'height' => array()),
            'td' => array( 'id' => array(), 'class' => array(), 'width' => array(), 'height' => array(), 'colspan' => array(), 'rowspan' => array() ),
            'th' => array( 'id' => array(), 'class' => array(), 'width' => array(), 'height' => array(), 'colspan' => array(), 'rowspan' => array() ),
            'tr' => array( 'id' => array(), 'class' => array(), 'width' => array(), 'height' => array(), 'colspan' => array(), 'rowspan' => array() ),
        ...
        $allowedHtml = array( 
            ...
            'br' => array(),
            'div' => array( 'id' => array(), 'class' => array() ),
            'input' => array( 'type' => array(), 'name' => array(), 'value' => array(), 'checked' => array(), 'id' => array(), 'class' => array(), 'placeholder' => array() ),
            'label' => array( 'id' => array(), 'class' => array(), 'for' => array() ),
            'p' => array(),
            'span' => array( 'id' => array(), 'class' => array() ),
            'strong' => array(),
...
function rvm_sanitize_post_value( $input, $type ) {    
    if ( is_array( $input ) ) {//for markers
        ...
        foreach ( $input as $key => $value ) { 
            switch ( $type ) {
                ...
                case 'url':
                    ...
                    break;
                case 'html':
                    $sanitized_input[ $key ] = wp_kses( $value, rvm_allowed_tags( 'html-popup' ) );
                    break;
                case 'num':
                    $sanitized_input[ $key ] = sanitize_text_field( $value );
                    break;
        ...
        switch ( $type ) {//for regions/subdivisions
            case 'text':
                $sanitized_input =  sanitize_text_field( $input );
                break;
            case 'url':
                $sanitized_input = esc_url( $input );
                break;
            case 'html':
                $sanitized_input = wp_kses( $input, rvm_allowed_tags( 'html-popup' ) );
                break;
            case 'num':
                $sanitized_input = sanitize_text_field( $input );
                break;
...
function rvm_maps_is_already_in_db( $map_id ) {
